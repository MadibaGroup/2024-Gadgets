...
- instead we interpolate two aux polynomials $\hat{f}$  through the points (r - f(a)) for $a \in H$ and product check these (this shows permutation)
- remains to show the aux polynomials were defined correctly; to do this for check $\hat{f}(xi) = r - f(xi) $. for intuition on why this holds: clearly it is true for $a \in \omega$ and $n$ points defined and $n-1$ degree polynomial, so holds for $\omega \iff$ holds for $\mathbb{F}$​
- this shows our desried relation
- for intuition on the first point see below (literally how I used to think it worked); it's like doing it with the two polynomials and checlling at a random point r:

-  Instead, the solution is to create two auxilary polynomials, ${\mathsf{Poly}}_\mathsf{Arr_1'}$ and ${\mathsf{Poly}}_\mathsf{Arr_2'}$ defined as follows: $\mathsf{Poly}_\mathsf{Arr_j'}(X) = \prod^{}_{}(X - \mathsf{Poly}_\mathsf{Arr_j}(z))$ for $z \in \mathcal{H}_\kappa$. If ${\mathsf{Poly}}_\mathsf{Arr_1'}$ = $\mathsf{Poly}_\mathsf{Arr_2'}$, then they have the same roots. This means that $\mathsf{Arr}_1$ and $\mathsf{Arr}_2$ must contain the same elements (in possibly different orders); in other words, $\mathsf{Arr}_2$ is a permutation of $\mathsf{Arr}_1$. To show  ${\mathsf{Poly}}_\mathsf{Arr_1'}$ = $\mathsf{Poly}_\mathsf{Arr_2'}$, a product check, specifically [mult3](./mult3), at a $X= r$ for a random challenge $r$ can be used. 
